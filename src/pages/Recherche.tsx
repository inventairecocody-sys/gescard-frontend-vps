...
const Recherche: React.FC = () => {
  ...
  const handleExportResultatsCSV = async () => {
    ...
    try {
      ...
      link.href = url; link.download = `resultats-recherche-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  const handleExportResultatsExcel = async () => {
    ...
    try {
      ...
      const url  = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url; link.download = `resultats-recherche-${new Date().toISOString().split('T')[0]}.xlsx`;
      document.body.appendChild(link); link.click(); document.body.removeChild(link);
      URL.revokeObjectURL(url);
      showToast('Export Excel des résultats téléchargé');
    } catch { showToast("Export Excel indisponible, utilisez le CSV", 'error'); }
    finally { setExportLoading(null); }
  };
  ...
  const handleExportToutCSV = async () => {
    ...
    try {
      ...
      const url  = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url; link.download = `export-complet-${new Date().toISOString().split('T')[0]}.csv`;
      document.body.appendChild(link); link.click(); document.body.removeChild(link);
      URL.revokeObjectURL(url);
      showToast('Export CSV complet téléchargé');
    } catch { showToast("Erreur lors de l'export CSV complet", 'error'); }
    finally { setExportLoading(null); }
  };
  ...
  const handleExportToutExcel = async () => {
    ...
    try {
      ...
      const url  = URL.createObjectURL(blob);
      const link = document.createElement('a');
      link.href = url; link.download = `export-complet-${new Date().toISOString().split('T')[0]}.xlsx`;
      document.body.appendChild(link); link.click(); document.body.removeChild(link);
      URL.revokeObjectURL(url);
      showToast('Export Excel complet téléchargé');
    } catch { showToast("Erreur lors de l'export Excel complet", 'error'); }
    finally { setExportLoading(null); }
  };
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-6 py-6 space-y-5">
        ...
        {loading && (
          ...
          >
            ...
            <div className="text-center">
              <p className="text-gray-700 font-semibold">Recherche en cours…</p>
              <p className="text-gray-400 text-sm mt-1">Interrogation de la base de données</p>
            </div>
          </motion.div>
        )}
        ...
        {!loading && resultats.length > 0 && (
          <motion.div
            ...
            className="bg-white rounded-2xl border border-gray-200 shadow-sm overflow-hidden"
          >
            {/* ── Header résultats avec pagination améliorée ── */}
            ...
            {/* ── Pagination bas de page (si beaucoup de résultats) ── */}
            {totalPages > 1 && (
              <div className="px-4 sm:px-5 py-3 border-t border-gray-100 bg-gray-50/40 flex items-center justify-between gap-2">
                <span className="text-xs text-gray-400">
                  Affichage {((currentPage - 1) * 50) + 1}–{Math.min(currentPage * 50, totalResultats)} sur {totalResultats.toLocaleString('fr-FR')}
                </span>
                <div className="flex items-center gap-2">
                  <button
                    onClick={() => handlePageChange(currentPage - 1)}
